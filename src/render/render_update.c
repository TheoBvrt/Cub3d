...
void	ray_calculator(t_game *game, t_raycasting *ray)
{
	...
	ray->text_pos = (ray->draw_start - game->s_h / 2 + ray->line_height / 2) * ray->step;
}
...
void	texture_selector(t_game *game, t_raycasting *ray)
{
	if (game->map.world_map[ray->map_x][ray->map_y] == 1)
	{
		...
		{
			if (ray->ray_dir_x  > 0)
				ray->tex_data = mlx_get_data_addr(game->image.texture, &ray->bpp, &ray->sl, &ray->end);
			else
				ray->tex_data = mlx_get_data_addr(game->image.stone, &ray->bpp, &ray->sl, &ray->end);
		}
		else
		{
			if (ray->ray_dir_y > 0)
				ray->tex_data = mlx_get_data_addr(game->image.brick, &ray->bpp, &ray->sl, &ray->end);
			else
				ray->tex_data = mlx_get_data_addr(game->image.coloredstone, &ray->bpp, &ray->sl, &ray->end);
		}
	}
}

void	buffer_feel(t_game *gane, t_raycasting *ray, unsigned int buffer[720][1280])
{
...
void	raycasting(t_game *game, t_raycasting *ray)
{
	...
	while (ray->x < game->s_w)
	{
		...
		while(ray->hit == 0)
		{
			...
			if(game->map.world_map[ray->map_x][ray->map_y] > 0) ray->hit = 1;
		}
